...
봇이 업무량과 스트레스 수준에 따라 성격을 동적으로 조절합니다. 백로그, 실패한 작업, 높은 활동량이 누적되면 봇의 분위기가 변합니다:

| 레벨 | 상태 | 행동 |
|------|------|------|
| 0 | 평온 | 일반 운영 |
| 1 | 바쁨 | 약간의 긴급감, 빠른 판단 |
| 2 | 압박 | 더 직접적/결단력 있는 대응 |
| 3 | 과부하 | 중요 작업만 우선 처리 |

스트레스는 시간이 지나면 자연 감쇠되며, 작업 실패·거부·타임아웃·큐 누적 등의 이벤트로 발생합니다. `teams.json`의 `stressProfile`로 에이전트별 민감도를 설정할 수 있습니다.

### AGENT.md (레포 컨텍스트)

저장소 루트에 `AGENT.md` 파일을 두면 해당 코드베이스의 AI 친화적 컨텍스트를 제공합니다. 에이전트가 해당 레포를 작업할 때 자동으로 감지되어 프롬프트에 주입됩니다 — 별도 설정 불필요.

주요 내용: 프로젝트 개요, 기술 스택, 아키텍처 요약, 브랜치 전략, 빌드/테스트 명령어, 네이밍 컨벤션. `AGENT.md`가 없으면 `prompts/repo-specific/{레포명}.md`로 폴백합니다.

### 압축 프롬프트 모드

같은 대화 내 반복 호출 시 자동으로 압축된 프롬프트를 사용하여 토큰 사용량을 ~37% 절감합니다. 첫 호출은 전체 프롬프트, 이후 호출은 예시와 중복 섹션을 제거하되 핵심 규칙은 유지합니다. 자동 감지 — 별도 설정 불필요.
...
| `stressProfile` | 스트레스 민감도, 감쇠율, 분위기 설명 커스터마이징 |
